...
# @register.simple_tag()  # указываем простой декоратор (simple_tag) взятия списка всех подразделений
# def get_division():   # задаем пользовательский тег
#     return Division.objects.annotate(cnt=Count('phonenumber')).filter(cnt__gt=0)  # отображает только те подразделения в которых есть записи


# @register.inclusion_tag('phonebook/list_divisions.html')  # указываем встроенный декоратор для списка подразделений в левом навбаре
...
@register.inclusion_tag('phonebook/list_divisions.html')  # указываем встроенный декоратор для списка подразделений в левом навбаре
def show_divisions():  # задаем пользовательский тег отобразить подразделения, передаем произвольный аргумент mil_id=2
    # divisions = Division.objects.all()  # загружает полный список таблицы divisions
    # divisions = Division.objects.annotate(cnt=Count('phonenumber')).filter(cnt__gt=0) #загружаем не пустые списки
    #divisions = Division.objects.annotate(cnt=Count('phonenumber', filter=F('phonenumber__is_published'))).filter(cnt__gt=0).filter(phonenumber__military_unit_id=1)
    # divisions = Division.objects.filter('phonenumber__military_unit_id')
...
# @register.inclusion_tag('phonebook/list_content.html')  # указываем встроенный декоратор используется с функциями
