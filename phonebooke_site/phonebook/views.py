...
### Контроллеры-классы
# создадить класс-обработчик главной страницы (переопределяем атрибуты класса ListView)
...
class PhonesByMilitary(ListView): # наследуем атрибуты от класса ListView
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context['title'] = MilitaryUnit.objects.get(pk=self.kwargs['mil_id'])  # mil_id - параметра запроса в urls
...
# создадить класс-обработчик страницы данных за подразделение (переопределяем атрибуты класса ListView)
class PhonesByDivision(ListView): # наследуем атрибуты от класса ListView
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context['title'] = Division.objects.get(pk=self.kwargs['div_id'])  # дополняем контекст тайтлом, div_id - параметра запроса в urls
        return context  # непосредственно передает словарь на страницу
